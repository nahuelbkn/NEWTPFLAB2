...
void buscarUsuario ()
{
    ...
    gotoxy(28,5);
    printf(" - Ingrese el ID del usuario que quiera ver: ");
    fflush(stdin);
    scanf("%i", &idMostrar);
...
void mostrarRegistroCancion (stCancion c, int clave)
{
    ...
    {
        gotoxy(0,0);
        printf("/// ----- INFO CANCION [%i] ----- ///", c.idCancion);
        gotoxy(3,1);
        printf("T%ctulo: %s", 161, c.titulo);
        gotoxy(3,2);
        printf("%clbum: %s", 181, c.album);
        gotoxy(3,3);
        printf("A%co: %i", 164, c.anio);
        gotoxy(3,4);
        printf("Artista: %s", c.artista);
        gotoxy(3,5);
        printf("Duraci%cn: %i", 162, c.duracion);
        gotoxy(3,6);
        printf("G%cnero: %s", 130, c.genero);
        gotoxy(3,7);
        printf("Comentario: %s", c.comentario);
        gotoxy(3,8);
        printf("Eliminado: %i\n", c.eliminado);
...
void modificarCancion (nodoArbolCancion * arbol)
{
    ...
    gotoxy(23,7);
    printf(" - Ingrese el ID de la canci%cn a modificar: ", 162);
    fflush(stdin);
    scanf("%i", &idModificar);
    ...
    {
        gotoxy(0, 15);
        printf("La canci%cn buscada no existe.", 162);
    }
}
...
void recomendados (nodoListaCancion * lista)
{
    ...
    {
        gotoxy(0,15);
        printf("No tienes canciones en tu playlist aun.");
        Sleep(1000);
    }
}
...
void reproducirPlayList(nodoListaCancion * listaUsuario)
{
    ...
    /*
    A saber:
        (Codigos ascii utilizados en esta función)
        - 97: Letra
        - 115: Letra
        - 27: Tecla ESC
    */
