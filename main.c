...
void primeraPantalla () {
    ...
    for (int i = 0; i<dimension; i++) {
        mostrarRegistroUsuario(ADL[i].cliente, 1);
    }
    system("pause");
...
void mostrarRegistroCancion (stCancion c, int clave) {
    ...
    } else if (clave == 5 || clave == 0) { /// CLIENTE accede por "opcion" o por nivel
        if (c.eliminado == 0) {
            puts ("\n\n/// ----- INFO CANCION ----- ///");
...
int menuPpalCliente () {
    ...
    gotoxy(3,9); printf(" Cerrar sesi%cn", 162);
    gotoxy(3,10); puts(" Salir");
